...
function Home() {
  ...
  const itemPrice = 800//Initial price of the product
  const [quantity, setQuantity] = useState(1);
  const[finalAmount, setFinalAmount] = useState(itemPrice);

  const increment = () => {
    setQuantity(quantity+1);//Onclick of the plus button, it adds a value(1) to the initial value(1)
    setFinalAmount(finalAmount+itemPrice);//Adds the 800 to the final amount
  }
      ...
      <div className='text-center font-raleway w-full max-w-5xl mx-auto my-6'>
        ...
        mx-auto w-full my-16 border-2 bg-[#fcf6f6] border-slate-100 shadow-md py-4'>
           ...
           <div className='flex flex-col lg:w-6/12 w-full py-8'>
             ...
             <span className='text-green-500 text-3xl font-bold pl-3'>N{finalAmount}</span></div>
             <div className='my-6'>
